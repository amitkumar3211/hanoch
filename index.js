...
const brokerAddress = process.env.MQTT_BROKER_ADDRESS;
const options = {
  clientId: process.env.MQTT_CLIENT_ID || "", // Default to empty string if not provided
  protocolVersion: parseInt(process.env.MQTT_PROTOCOL_VERSION, 10),
...
// let topics = ["data/ES-08:F9:E0:8F:26:04"];
...
client.on("connect", (connack) => {
  if (connack.returnCode === 0) {
    ...
    console.error(
      "Failed to connect to MQTT broker. Return code:",
      connack.returnCode
    );
  }
});
    ...
    try {
      ...
      if (machineName) {
        ...
        if (isComplete) {
          ...
          // Save the document to the database
          const newData = new MQTTDataModel({
            machineName,
...
app.get("/api/machine/data", verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.userId)
      .populate("company_name") // Populate the 'company_name' field with 'company_name' property
      .populate("machine_name");
    if (user && user.machine_name && user.machine_name.topic) {
      ...
      const machines = await DataModel.find({ topic: user_topic }).populate(
        "modbusRTU"
      );
      ...
      const filteredData = machines.filter((machine) => {
        if (!machine.datetime) {
          // console.error(`Missing datetime for machine ID: ${machine._id}`);
          return false;
        }
        console.log(machine.datetime);
        ...
        if (!machineDateTime.isValid()) {
          console.error(`Invalid date format: ${machine.datetime}`);
          return false;
        }
      ...
      console.log(moment(machines[0].ts).format("MM/DD/YYYY HH:mm:ss"));

      let good_parts = 0;
      let bad_parts = 0;
      let total_parts = 0;

      good_parts = filteredData[filteredData.length - 1]?.modbusRTU?.get("1")
        ?.data?.[0]?.[0]
        ? filteredData[0]?.modbusRTU?.get("1")?.data?.[0]?.[0]
        : 0;
      bad_parts = filteredData[filteredData.length - 1]?.modbusRTU?.get("1")
        ?.data?.[1]?.[0]
        ? filteredData[0]?.modbusRTU?.get("1")?.data?.[1]?.[0]
        : 0;
      total_parts = filteredData[filteredData.length - 1]?.modbusRTU?.get("1")
        ?.data?.[2]?.[0]
        ? filteredData[filteredData.length - 1]?.modbusRTU?.get("1")
            ?.data?.[2]?.[0]
        : 0;
      ...
      for (const entry of filteredData) {
        ...
        if (entry.DIN[1] === 1 && lastStatus === 0) {
          ...
          totalRunningHours += entryDateTime.diff(
      ...
      const filteredDataShiftAgo = machines.filter((machine) => {
        ...
        if (
          machineDateTime.isSameOrAfter(eightHoursAgo) &&
          machineDateTime.isSameOrBefore(currentTime)
        ) {
          return true;
        }
        return false;
      ...
      good_parts_ago = filteredDataShiftAgo[
        filteredDataShiftAgo.length - 1
      ]?.modbusRTU?.get("1")?.data?.[0]?.[0]
        ? filteredDataShiftAgo[0]?.modbusRTU?.get("1")?.data?.[0]?.[0]
        : 0;
      bad_parts_ago = filteredDataShiftAgo[
        filteredDataShiftAgo.length - 1
      ]?.modbusRTU?.get("1")?.data?.[1]?.[0]
        ? filteredDataShiftAgo[0]?.modbusRTU?.get("1")?.data?.[1]?.[0]
        : 0;
      total_parts_ago = filteredDataShiftAgo[
        filteredDataShiftAgo.length - 1
      ]?.modbusRTU?.get("1")?.data?.[2]?.[0]
        ? filteredDataShiftAgo[filteredDataShiftAgo.length - 1]?.modbusRTU?.get(
            "1"
          )?.data?.[2]?.[0]
        : 0;
      ...
      for (const entry of filteredDataShiftAgo) {
        ...
        if (entry.DIN[1] === 1 && lastStatusAgo === 0) {
          ...
          // Transition from on to off
          totalRunningHoursAgo += entryDateTime.diff(
            lastStatusChangeTimeAgo,
            "hours",
            true
          );
        }
      ...
      if (lastStatusAgo === 1 && lastStatusChangeTimeAgo) {
        totalRunningHoursAgo += moment().diff(
          lastStatusChangeTimeAgo,
          "hours",
          true
        );
      ...
      if (timeRange === "6:00 - 14:00") {
        ...
        const filteredData2ShiftAgo = machines.filter((machine) => {
          ...
          // Check if the machineDateTime is within the last 8 hours
          if (
            machineDateTime.isSameOrAfter(eight2HoursAgo) &&
            machineDateTime.isSameOrBefore(currentTime)
          ) {
            return true;
          }
        ...
        total_parts_2st_shift =
          filteredData2ShiftAgo[
            filteredData2ShiftAgo.length - 1
          ]?.modbusRTU?.get("1")?.data?.[2]?.[0];
        ...
        const filteredData3ShiftAgo = machines.filter((machine) => {
          ...
          // Check if the machineDateTime is within the last 8 hours
          if (
            machineDateTime.isSameOrAfter(eight3HoursAgo) &&
            machineDateTime.isSameOrBefore(currentTime)
          ) {
            return true;
          }
        ...
        total_parts_3st_shift =
          filteredData3ShiftAgo[
            filteredData3ShiftAgo.length - 1
          ]?.modbusRTU?.get("1")?.data?.[2]?.[0];
        ...
        const filteredData1ShiftAgo = machines.filter((machine) => {
          ...
          // Check if the machineDateTime is within the last 8 hours
          if (
            machineDateTime.isSameOrAfter(eight1HoursAgo) &&
            machineDateTime.isSameOrBefore(currentTime)
          ) {
            return true;
          }
        ...
        total_parts_1st_shift =
          filteredData1ShiftAgo[
            filteredData1ShiftAgo.length - 1
          ]?.modbusRTU?.get("1")?.data?.[2]?.[0];
      ...
      const planed_production_time = user.machine_name.production_time
        ? user.machine_name.production_time
        : 8;
      const ideal_cycle_time = user.machine_name.ideal_cycle_time
        ? user.machine_name.ideal_cycle_time
        : 8;
      const scrap_number = user.machine_name.scrap_number
        ? user.machine_name.scrap_number
        : 8;
      const design_capacity = user.machine_name.design_capacity
        ? user.machine_name.design_capacity
        : 8;
  ...
  try {
    ...
    const calculateMetricsForShift = (dataList) => {
      ...
      dataList.forEach((data) => {
        ...
        const good_parts = data?.modbusRTU?.get("1")?.data?.[0]?.[0]
          ? data.modbusRTU.get("1").data[0][0]
          : 0;
        const bad_parts = data?.modbusRTU?.get("1")?.data?.[1]?.[0]
          ? data.modbusRTU.get("1").data[1][0]
          : 0;
        const total_parts = data?.modbusRTU?.get("1")?.data?.[2]?.[0]
          ? data.modbusRTU.get("1").data[2][0]
          : 0;
    ...
    const shift6to14 = lastDataList.filter((data) => {
      ...
      return (
        moment(data.datetime, "DD/MM/YYYY HH:mm:ss").hour() >= 6 &&
        moment(data.datetime, "DD/MM/YYYY HH:mm:ss").hour() < 14
      );
    });
    ...
    const shift14to22 = lastDataList.filter((data) => {
      ...
      return (
        moment(data.datetime, "DD/MM/YYYY HH:mm:ss").hour() >= 14 &&
        moment(data.datetime, "DD/MM/YYYY HH:mm:ss").hour() < 22
      );
    });
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (email === "admin@gmail.com" && password === "test1234") {
      ...
      token = jwt.sign({ userId: user._id, admin: true }, JWT_SECRET, {
        expiresIn: "2d",
      });
      res.status(200).json({
...
app.get("/api/user/profile", verifyToken, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.userId).populate(
      "company_name",
      "company_logo"
    ); // Populate the 'company_name' field with 'company_name' property
...
app.get("/api/machine/:id", verifyToken, async (req, res) => {
  try {
  ...
  try {
    ...
    if (user && user.machine_name && user.machine_name.topic) {
      ...
      const machines = await Machine.find({
        company_name: user.company_name._id,
        topic: userTopic,
      });

      if (!machines || machines.length === 0) {
        return res.status(404).json({
          error: "No machines found for the user's company and topic.",
        });
      }
...
app.get("/api/all/machine/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (user.role === "admin") {
      ...
      const machines = await Machine.find({ company_name: id }).populate(
        "company_name",
        "company_name"
      );
      //console.log(machines);
  ...
  async (req, res) => {
    ...
    if (user.role === "admin") {
      try {
        ...
        // If company name is provided, find the corresponding company
        let companyId = updateData.company_name;
        if (
          updateData.company_name &&
          typeof updateData.company_name === "string"
        ) {
        ...
        if (companyId) selectiveUpdate.company_name = companyId;
        if (updateData.production_time) {
          selectiveUpdate.production_time = updateData.production_time;
        }
        if (updateData.topic) {
          selectiveUpdate.topic = updateData.topic;
        }
        if (updateData.ideal_cycle_time) {
          selectiveUpdate.ideal_cycle_time = updateData.ideal_cycle_time;
        }
        if (updateData.planned_shutdown) {
          selectiveUpdate.planned_shutdown = updateData.planned_shutdown;
        }
        if (updateData.scrap_number) {
          selectiveUpdate.scrap_number = updateData.scrap_number;
        }
        if (updateData.design_capacity) {
          selectiveUpdate.design_capacity = updateData.design_capacity;
        }
        if (updateData.machine_name) {
          selectiveUpdate.machine_name = updateData.machine_name;
        }
        if (req.file?.filename) {
          selectiveUpdate.logo = req.file.filename;
        }
    ...
    if (user.role === "admin") {
      try {
        ...
        if (updateData.company_name) {
          selectiveUpdate.company_name = updateData.company_name;
        }
        if (req.file?.filename) {
          selectiveUpdate.company_logo = req.file.filename;
        }
        // Add other fields as needed
...
// Child Parts Related APIs
app.post("/create/childPart", verifyToken, async (req, res) => {
  try {
    // Parse form data from the request body
    const { machine, childParts } = req.body;

    const user = await User.findById(req.userId);
    if (user.role === "admin") {
      const machineName = await Machine.findOne({ machine_name: machine });
      // console.log("this is machineName", machineName);

      if (!machineName) {
        return res.status(404).json({ error: "Machine not found" });
      }
      // Create a new document based on the FormData model
      const newChildPart = new childPart({
        machine: machineName._id,
        childParts: childParts.map((part) => ({ value: part.value })), // Map childParts to the correct structure
      });

      // Save the document to the database
      await newChildPart.save();

      res.status(201).json({ message: "Child Part Added successfully" });
    } else {
      return res.status(401).json({ error: "Unauthorized - Invalid token" });
    }
  } catch (error) {
...
app.get("/api/all/childParts", verifyToken, async (req, res) => {
  try {
    //console.log(req.body);
    // const { company_name } = req.body;
    //console.log(req.userId);
    //const user = await User.findById(req.userId);
    // if (user.email === 'admin@gmail.com') {
    const childParts = await childPart.find({}).populate("machine");
    // console.log(childParts);

    res.status(200).json(childParts);
    //}
    // else {
    //    return res.status(401).json({ error: 'Unauthorized - Invalid token' });
    // }
  } catch (err) {
    console.error("Error finding childParts:", err);
    res.status(500).json({ error: "Internal Server Error" });
  }
});

app.get("/api/all/childParts/:id", verifyToken, async (req, res) => {
  try {
    //console.log(req.userId);
    const user = await User.findById(req.userId);
    if (user.role === "admin") {
      ...
      // console.log(id);
      // Assuming your Machine model has a reference to the Company model
      const childParts = await childPart
        .find({ machine: id })
        .populate("childParts");
...
app.patch("/api/all/childParts/:id", verifyToken, async (req, res) => {
  const _id = req.params.id;
  const updateData = req.body;
  ...
  if (user.role === "admin") {
    try {
      ...
      // If machine name is provided, find the corresponding machine
      let machineId = updateData.machine;
      if (updateData.machine && typeof updateData.machine === "string") {
        const machine = await Machine.findOne({
          machine_name: updateData.machine,
        });
        if (!machine) {
          return res.status(404).json({ error: "Machine not found" });
        }
      ...
      // Build a selective update object
      const selectiveUpdate = {};
      if (machineId) selectiveUpdate.machine = machineId;
      if (updateData.childParts && Array.isArray(updateData.childParts)) {
        selectiveUpdate.childParts = updateData.childParts.map((part) => ({
          value: part.value,
        }));
      }

      // Update the child part data in the database
      const updatedChildParts = await childPart.findByIdAndUpdate(
        _id,
        selectiveUpdate,
        { new: true }
      );

      if (!updatedChildParts) {
        return res.status(404).json({ error: "ChildPart not found" });
      }

      res.json({ message: "ChildPart updated successfully" });
    } catch (error) {
      console.error("Error updating ChildPart:", error);
      res.status(500).json({ error: "Internal Server Error" });
    }
  } else {
    return res.status(401).json({ error: "Unauthorized - Invalid token" });
...
app.delete(
  "/api/delete/childParts/:childPartsId",
  verifyToken,
  async (req, res) => {
    try {
      //console.log(req.body);
      ...
      const user = await User.findById(req.userId);
      // if (user.role === "admin") {
      const childPartsId = req.params.childPartsId;
      //console.log(machineId);
      // Find the user by ID and delete
      const deletedChildPart = await childPart.findByIdAndDelete(childPartsId);
...
// Inventory Details Related APIs
app.post("/create/inventory", verifyToken, async (req, res) => {
  // console.log("this is req.body", req.body);
  try {
    // Parse form data from the request body
    const { machine, partName, stock } = req.body;

    const user = await User.findById(req.userId);
    if (user.role === "admin") {
      const machineName = await Machine.findOne({ _id: machine });
...
app.get("/api/all/inventory", verifyToken, async (req, res) => {
  try {
    //console.log(req.body);
    // const { company_name } = req.body;
    //console.log(req.userId);
    //const user = await User.findById(req.userId);
    // if (user.email === 'admin@gmail.com') {
    const inventory = await Inventory.find({}).populate("machine");
...
app.patch("/api/all/add/inventory/:id", verifyToken, async (req, res) => {
  const _id = req.params.id;
  const updateData = req.body;
  ...
  try {
    ...
    // Retrieve the inventory object from the database
    const existingInventory = await Inventory.findById(_id);
    ...
    const updatedStock = existingInventory.stock + updateData.stock * 1;

    // console.log("Update data:", updatedStock);

    // Build a selective update object
    const selectiveUpdate = { stock: updatedStock };

    // Validate and update the inventory data in the database
...
app.patch("/api/all/remove/inventory/:id", verifyToken, async (req, res) => {
  const _id = req.params.id;
  const updateData = req.body;
...
//Batch Control related APIs
app.post("/create/batchControl", verifyToken, async (req, res) => {
  console.log("this is req.body", req.body);
  try {
    const user = await User.findById(req.userId);

    // if (user.role === "batch_control_manager") {
    const machineName = await Machine.findOne({ _id: req.body.machine });

    if (!machineName) {
      return res.status(404).json({
        error: "Machine not found, Please select Machine and it's part",
      });
    }

    // Create a new document based on the FormData model
    const newBatch = new BatchControl({
      operator: req.body.operator,
      machine: req.body.machine,
      partName: req.body.partName,
      materialUsed: req.body.materialUsed,
      cavityNo: req.body.cavityNo,
      burnMark: req.body.burnMark,
      discolouration: req.body.discolouration,
      spillage: req.body.spillage,
      dentMark: req.body.dentMark,
      sortRejections: req.body.sortRejections,
      flash: req.body.flash,
      setUp: req.body.setUp,
      shortShots: req.body.shortShots,
...
app.get("/api/all/batchControl", verifyToken, async (req, res) => {
  try {
    ...
    //const user = await User.findById(req.userId);
    // if (user.email === 'admin@gmail.com') {
    const batchControl = await BatchControl.find({}).populate("machine");
    // console.log(inventory);

    res.status(200).json(batchControl);
    //}
    // else {
    //    return res.status(401).json({ error: 'Unauthorized - Invalid token' });
    // }
...
app.delete("/api/delete/batch/:batchId", verifyToken, async (req, res) => {
  try {
    const user = await User.findById(req.userId);
    // if (user.role === "admin") {
    ...
    console.log(req.params);
    // Find the user by ID and delete
    const deletedBatch = await BatchControl.findByIdAndDelete(batchId);
...
//Production Planning related APIs
